...
const calculateEdgePoints = ( sourceNode: Node, targetNode: Node ): number[] => {
  ...
  const radius = sourceNode.shape.radius;
  ...
  const sourcePointX = sourceX + nx * radius;
  const sourcePointY = sourceY + ny * radius;
  const targetPointX = targetX - nx * radius;
  const targetPointY = targetY - ny * radius;
...
const NodeComponent: FC<NodeProps> = ( {
  ...
  const { radius, edge, position, color } = node.shape;
  return (
    <Group
      key={ node.id }
      x={ position.x || 0 }
...
export function View() {
  ...
  const handleStageDoubleClick = useCallback( ( e: KonvaEventObject<MouseEvent> ) => {
    ...
    addNode( {
      ...
      label: `Node ${localGraph.nodes.length + 1}`,
      parentId: localGraph.currentRoot?.id,
      metadata: {}, // Empty metadata for key_value node
      shape: {
        position: {
          x: pointerPosition.x,
          y: pointerPosition.y
  ...
  }, [ addNode, localGraph.nodes.length, localGraph.currentRoot?.id ] );
